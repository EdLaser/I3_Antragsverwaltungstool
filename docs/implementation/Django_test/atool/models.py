...
class Finance(models.Model):
    ...
    def __str__(self):
        ...
        self.reason, self.budget, self.suggestion, self.mehrheit, self.beschluss, self.beschlusstext,
        self.beschlussgrund)
...
class Position(models.Model):
    ...
    def __str__(self):
        return '%s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s' % (
            self.flag, self.number, self.date, self.title, self.office, self.name, self.mail, self.text, self.frg1, self.frg2, self.frg3, self.frg4, self.frg_spez_1, self.frg_spez_2, self.frg_spez_3, self.mehrheit,
            self.beschluss, self.beschlusstext, self.beschlussgrund)
...
class Universall(models.Model):
    ...
    def __str__(self):
        ...
        self.suggestion, self.mehrheit, self.beschluss, self.beschlusstext, self.beschlussgrund)
