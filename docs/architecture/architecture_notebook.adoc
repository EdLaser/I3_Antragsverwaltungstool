...
. Wir verwenden PHP, da es ein bekannter Standard zur weitergabe von Nutzereingaben ist.
. Um die Nutzereingaben serverseitig zu Bearbeiten, verwenden wir Python als gut verständliche Programmiersprache.
...
. Um die von PHP übermittelten Eingaben in einen Datensatz in der Datenbank umzuwandeln, nutzen wir die PyMysql um aus den Eingaben Abfragen auf der Datenbank zu generieren.
